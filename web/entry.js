      ...
      <Switch>
        {
          Routes.map(({ path, exact, Component }) => {
            ...
            ActiveComponent.HeaderNav = ActiveComponent.HeaderNav || [{ path: '/news/1', name: '導航' }]
            return <Route exact={exact} key={path} path={path} render={() => <Layout headerNav={ActiveComponent.HeaderNav}><WrappedComponent store={store} /></Layout>} />
          })
        }
      </Switch>
    </BrowserRouter>
...
const serverRender = async ctx => {
  ...
  app.router(() => (
    <StaticRouter location={ctx.req.url} context={storeState}>
      <Layout layoutData={ctx} headerNav={ActiveComponent.HeaderNav}>
